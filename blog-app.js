import { collection, limit, onSnapshot, query, where } from "https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js";
import { db } from "./firebase-app.js";

const state = { blogs: [], category: "All", query: "", unsubscribe: null };
...
function blogDate(blog) { return formatDate(blog.publishedAt || blog.publishDate || blog.date); }
function sortTime(blog) {
  if (blog.publishedAt?.toMillis) return blog.publishedAt.toMillis();
  const date = blog.publishDate || blog.date;
...
function renderBlogs(data) {
  ...
  data.forEach((blog) => {
    ...
    badge.textContent = blog.category || "Health";
    ...
    excerpt.textContent = blog.excerpt || "Read this health and wellness article from NutriLife AI.";
    const footer = document.createElement("div");
    footer.className = "card-footer";
    footer.innerHTML = `<span>Read Article</span><i class="fas fa-arrow-right"></i>`;

    content.append(meta, title, excerpt, footer);
    card.append(wrapper, content);
    card.addEventListener("click", () => openModal(blog));
    card.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openModal(blog); }
    });
...
function renderCategories() {
  ...
  const categories = [...new Set(state.blogs.map(b => String(b.category || "").trim()).filter(Boolean))].sort((a,b) => a.localeCompare(b));
  const allCategories = ["All", ...categories];
  if (!allCategories.includes(state.category)) state.category = "All";

  allCategories.forEach(category => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = `cat-btn ${category === state.category ? "active" : ""}`;
    btn.dataset.category = category;
    btn.textContent = category;
    btn.addEventListener("click", () => {
      state.category = category;
      document.querySelectorAll("#categoryContainer [data-category]").forEach(b => b.classList.toggle("active", b === btn));
      applyFilters();
    });
...
function applyFilters() {
  ...
  const filtered = state.blogs.filter((b) => {
    ...
    const text = `${b.title || ""} ${b.excerpt || ""} ${b.category || ""} ${b.content || ""}`.toLowerCase();
...
function openModal(blog) {
  ...
  if (desc) desc.content = blog.seoDescription || blog.excerpt || desc.content;
  const url = new URL(window.location.href);
  url.searchParams.set("post", blog.slug || blog.id);
  history.replaceState({}, "", url);
}

function handleSnapshot(snap) {
  state.blogs = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  state.blogs.sort((a, b) => sortTime(b) - sortTime(a));
  renderCategories();
  hideStatus();
  applyFilters();
  ...
  if (post) {
    const found = state.blogs.find(b => (b.slug || b.id) === post);
...
function loadBlogs() {
  ...
  try {
    ...
    state.unsubscribe = onSnapshot(
      ...
      handleSnapshot,
      (error) => {
        console.error("NutriLife AI blog Firestore error:", error);
        const message = error?.code === "permission-denied"
          ? "Blog access was blocked by Firebase security rules. Please check Firestore rules."
          : "We could not load the articles right now. Please refresh the page or try again later.";
    ...
    console.error("NutriLife AI blog initialization error:", error);
    setStatus("The blog could not start. Please refresh the page and try again.", "error");
  }
}
...
function closeModal() {
  ...
  const desc = document.querySelector('meta[name="description"]');
  if (desc) desc.content = "Science-backed health, nutrition, wellness, weight management, and lifestyle articles from NutriLife AI.";
  const url = new URL(window.location.href);
